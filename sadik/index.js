...
var images = {
	'zed_grafiti'	:{'url':'zed2.jpg','size':195.6},
...
//		$('#fps').html('UP');
		var y = Math.floor(Math.max(0,(c_h - h - $(document).scrollTop())/w)* 1000)/1000;
		var y2 = Math.floor((y - h / w )*1000)/1000; 
...
//			alert(id+':'+this);
	...
	// setInterval(on_scroll,40);
			var $modal = $('#page_info > div');
	var $modalCallElems = $('[data-modal]');
	var $pageInfo = $('#page_info');
	var $pageInfoImg = $('img', $pageInfo);
	var $pageInfoHeader = $('h2', $pageInfo);
	var $pageInfoLink = $('a.mail', $pageInfo);
	$modal.append('<p class="modal-text" />');
	var $modalText = $('p.modal-text', $modal);

	$modalCallElems.on('click', function(e){
		e.preventDefault();
		var $elem = $(this);
		$pageInfoImg.attr('src', $elem.attr('data-modal-photo'));
		$pageInfoHeader.text($elem.attr('data-modal-header'));
		$pageInfoLink.text($elem.attr('data-modal-link-text'))
		$modalText.text($elem.attr('data-modal-text'));
		show_info();
	})
...
//			alert(id+':'+this);
	...
	$modalCallElems.on('click', function(e){
		...
		$pageInfoHeader.text($elem.attr('data-modal-header'));
		$pageInfoLink.text($elem.attr('data-modal-link-text'));
		$pageInfoLink.attr('href', $elem.attr('data-modal-link-href'));
		$modalText.text($elem.attr('data-modal-text'));
		show_info();
	})
